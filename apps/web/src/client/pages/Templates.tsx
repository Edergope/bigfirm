...
export function Templates() {
  ...
  return <div className="pb-2">
    ...
    <div className={clsx("grid min-h-[650px] overflow-hidden rounded-[var(--radius-lg)] bg-iusia-paper shadow-[var(--shadow-surface)]", catalogOpen ? "lg:grid-cols-[minmax(260px,300px)_minmax(0,1fr)] 2xl:grid-cols-[minmax(260px,320px)_minmax(0,1fr)]" : "lg:grid-cols-[44px_minmax(0,1fr)]")}>
...
function TemplateInspector({ template, superadmin, onNewVersion }: { template: TemplateRow; superadmin: boolean; onNewVersion: () => void }) {
  const queryClient = useQueryClient();
  const status = useMutation({ mutationFn: (value: "ACTIVE" | "INACTIVE" | "RETIRED") => api.setTemplateStatus(template.id, value), onSuccess: async () => { await Promise.all([queryClient.invalidateQueries({ queryKey: ["admin-templates"] }), queryClient.invalidateQueries({ queryKey: ["templates"] })]); } });
  return <div className="flex min-h-[650px] flex-col">
    <header className="border-b border-iusia-mist/20 bg-iusia-paper px-4 py-3">
      ...
      {superadmin ? <div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="secondary" onClick={onNewVersion}><History size={13} /> Nueva versión</Button>{template.status !== "ACTIVE" ? <Button size="sm" variant="secondary" onClick={() => status.mutate("ACTIVE")} disabled={status.isPending}><Power size={13} /> Activar</Button> : <Button size="sm" variant="secondary" onClick={() => status.mutate("INACTIVE")} disabled={status.isPending}><Power size={13} /> Desactivar</Button>}<Button size="sm" variant="secondary" onClick={() => status.mutate("RETIRED")} disabled={status.isPending || template.status === "RETIRED"}><Archive size={13} /> Retirar</Button></div> : null}
    </header>
