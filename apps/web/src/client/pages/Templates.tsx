...
function TemplateInspector({ template, superadmin, onNewVersion }: { template: TemplateRow; superadmin: boolean; onNewVersion: () => void }) {
  ...
  const status = useMutation({ mutationFn: (value: "ACTIVE" | "INACTIVE" | "RETIRED") => api.setTemplateStatus(template.id, value), onSuccess: async (result) => { await Promise.all([queryClient.refetchQueries({ queryKey: ["admin-templates"] }), queryClient.refetchQueries({ queryKey: ["templates"] })]); setNotice(result.status === "ACTIVE" ? "Plantilla activada." : "Estado de plantilla actualizado."); }, onError: () => setNotice("No fue posible activar la plantilla.") });
