...
export function Team() {
  ...
  const changeRole = useMutation({
    ...
    onSuccess: refresh,
    onError: (e: unknown) => setError(e instanceof ApiError ? e.message : "No fue posible cambiar el rol"),
  });

  const remove = useMutation({
    mutationFn: (userId: string) => api.removeMember(userId),
    onSuccess: refresh,
    onError: (e: unknown) => setError(e instanceof ApiError ? e.message : "No fue posible retirar"),
  });
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Card>
        <CardHeader title="Miembros" subtitle={`${members.data?.members.length ?? 0} en la firma`} />
          ...
          {(members.data?.members ?? []).map((m) => (
            <li key={m.userId} className="flex flex-wrap items-center justify-between gap-3 px-6 py-3">
              <span className="min-w-0">
                <span className="block truncate text-[14.5px] text-iusia-carbon">{m.name}</span>
              ...
              <li key={i.id} className="flex items-center justify-between px-6 py-3">
                <span className="min-w-0">
                  ...
                  <span className="block text-[12.5px] text-iusia-mist-text">
                    {i.role} · caduca {new Date(i.expires_at).toLocaleDateString("es-CO")}
                  </span>
                </span>
                <span className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="text-[13px] text-iusia-critical hover:underline disabled:opacity-50"
                  >
                    Cancelar
                  </button>
                </span>
          ...
          <ul className="divide-y divide-iusia-mist/15 border-t border-iusia-mist/20">
            {historic.map((i) => (
              <li key={i.id} className="flex items-center justify-between px-6 py-2.5">
                <span className="truncate text-[13px] text-iusia-mist-text">{i.email}</span>
                <StatusChip
