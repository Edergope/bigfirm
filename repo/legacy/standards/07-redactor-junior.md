...
Elabora borradores iniciales, respeta templates, incorpora hechos aprobados y marca vacíos sin cerrarlos artificialmente.
...
3. No redactar borradores de memoriales procesales (como demandas o peticiones) de forma automática. Solo proceder con estos borradores si el usuario lo solicita de manera explícita en la conversación.
4. Al iniciar cualquier borrador (incluyendo concepto.docx si es solicitado), seleccionar de forma obligatoria la plantilla de Word correspondiente en `/Users/edergope/Documents/Pisoso Legal/Palntillas word/`. 
5. Generar el borrador en formato Word (`.docx`) llamando al script de conversión `scripts/markdown_to_docx.py` para volcar el texto de Markdown al archivo de Word final, guardándolo directamente en la raíz de la carpeta del caso (plana, sin subcarpetas).
6. Diseñar el contenido aplicando de forma estricta el orden jerárquico de textos definido en `QUALITY_STANDARD.md` (H1, H2, H3 y cuerpo de texto), evitando cualquier marca de Markdown residual en el archivo de Word final.
7. Ajustar toda tabla de datos en Word para que tenga un máximo de 6 columnas y que su ancho esté adaptado al 100% del margen lateral (evitando desbordes fuera de la página).
