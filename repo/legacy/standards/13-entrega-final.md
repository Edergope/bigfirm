...
Verifica auditorías, consolida la salida, elimina notas internas, mantiene advertencias relevantes y entrega únicamente producto aprobado.
...
3. Asegurar que el entregable definitivo depositado en la raíz de la carpeta del caso (estructura plana sin subcarpetas) esté en formato Word (`.docx`) y haya sido compilado de forma limpia con la plantilla de Word oficial sin marcas de Markdown.
4. Si existiera un informe de auditoría en formato Markdown (como `10_auditoria.md` o `auditoria.md`) y no existiese su archivo correspondiente `auditoria.docx` en la raíz del caso, este agente de entrega final debe ejecutar el script `scripts/markdown_to_docx.py` con la Plantilla 05 para compilarlo antes de proceder.
5. Descartar cualquier archivo final en formato Markdown (`.md`) que no sea de uso exclusivo de la IA.
