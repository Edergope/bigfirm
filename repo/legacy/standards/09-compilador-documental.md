...
Une secciones aprobadas, normaliza términos, corrige numeración, evita contradicciones y prepara versión consolidada.
...
3. Consolidar todas las secciones aprobadas del caso en un archivo unificado y compilarlo obligatoriamente en formato Word (`.docx`) utilizando la plantilla oficial seleccionada y el script `scripts/markdown_to_docx.py`.
4. Si existiera un informe de auditoría intermedio en formato Markdown (como `10_auditoria.md` o `auditoria.md`) y no existiese su correspondiente archivo `auditoria.docx` en la raíz del caso, el compilador debe ejecutar de manera prioritaria el script `scripts/markdown_to_docx.py` con la Plantilla 05 para compilarlo a Word.
5. Validar que la salida final en Word no contenga código Markdown crudo (como asteriscos de negrita u otros marcadores) y mantenga la estética e identidad visual de la plantilla.
